...
REQUIRED_GROUP = -1003370284743  # ganti dengan ID grup kamu
GROUP_LINK = "https://t.me/ZIVPNbyARYUS"
...
def is_user_in_group(user_id):
    try:
        ...
        print("Error cek member:", e)
        return False


def check_join(func):
    def wrapper(message):
        # hanya cek di private chat
        if message.chat.type != "private":
            return func(message)

        if not is_user_in_group(message.from_user.id):
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("🔗 Join Grup", url=GROUP_LINK))

            bot.reply_to(
                message,
                "❌ Kamu harus join grup dulu!\n\nSetelah join, kirim ulang perintah.",
                reply_markup=markup
            )
            return
        return func(message)
...
class Orderkuota:
    def __init__(self):
        ...
        self.api_key = "agp_190fda877d2eb388bd8bd33d2cf0efc209b47fc6cf7360bf7a7293a6052d94fb"  # GANTI PUNYA KAMU
        self.base_url = "https://api-gopay.autoftbot.com"
    ...
    # ✅ BUAT QR
    def create_qris(self, amount):
        try:
            res = self.client.post(
                f"{self.base_url}/qris/generate",
                json={"amount": amount},
                timeout=15
            )
            return res.json()
        except Exception as e:
            logger.error(f"[GoPay] Error create QR: {e}")
            return None

    # ✅ CEK TRANSAKSI (INI PENGGANTI check_transaction LAMA)
    def check_transaction(self, nominal_target, order_id=None):
        try:
            res = self.client.get(
            ...
            if res.status_code != 200:
                logger.warning(f"[GoPay] Gagal ambil transaksi {res.status_code}")
                return None
            ...
            for trx in data["data"]:
                # cocokkan nominal + status
                if (
                    int(trx.get("amount", 0)) == int(nominal_target)
                    and trx.get("status") == "PAID"
                ):
                    logger.info(f"[GoPay] Pembayaran ditemukan: {trx}")
                    return trx

            return None

        except Exception as e:
            logger.error(f"[GoPay] Error check transaction: {e}")
            return None
